...
class PropertyUnggulanController extends Controller
{
    ...
    {
        $property = PropertyUnggulan::with(
            "data:id,slug,judul,tipe_properti,harga,luas_tanah,kamar_mandi,kamar_tidur,agent_id,created_at,area",
            "data.creator",
            "data.images"
        )->paginate(6);
        return response()->json(
    ...
    {
        $property = PropertyUnggulan::where("highlight", 1)
            ->with(
                "data:id,slug,judul,tipe_properti,harga,luas_tanah,kamar_mandi,kamar_tidur,agent_id,created_at,area,luas_bangunan",
                "data.creator",
                "data.images"
            )
            ->get();
